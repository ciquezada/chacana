...
class Catalog():
    ...
    def _worker(self, args):
        ...
        star_data["star_alt"] = star_data["alt"]
        star_data["star_az"] = star_data["az"]
        return nearest_pixel.append(star_data[["star_name", "ra", "dec", "star_alt", "star_az"]])
...
class CatalogImageHandler(ImageHandler):
    ...
    def _color_red(self, x, y):
        for i in range(3):
            for j in range(3):
                self._image_preview[int(y-1+j),int(x-1+i),0] = 250
                self._image_preview[int(y-1+j),int(x-1+i),1] = 0
                self._image_preview[int(y-1+j),int(x-1+i),2] = 0

    def _color_green(self, x, y):
        for i in range(3):
            for j in range(3):
                self._image_preview[int(y-1+j),int(x-1+i),0] = 0
                self._image_preview[int(y-1+j),int(x-1+i),1] = 250
                self._image_preview[int(y-1+j),int(x-1+i),2] = 0
    ...
    def get_image(self):
        if not isinstance(self._image_preview, type(np.array([]))):
            self.create()
        out = self._image_preview
        self._image_preview = None
        return out

    def update_catalog_pixtab(self):
        if not isinstance(self._image_preview, type(np.array([]))):
            self.create()
        new_df = self._pixels.copy()
        for indx in new_df.index:
            ...
            new_x = input("X: ")
            new_y = input("Y: ")
            new_df.at[indx, "x"] = new_x
            new_df.at[indx, "y"] = new_y
            self._color_green(x, y)
        return new_df

if __name__=="__main__":
    CATALOG_PATH = "bsc5.dat"
    # PIXTAB_PATH = "09-10-2019_05h24m41s_Interpolated_Data_handcraft.pixtab"
    PIXTAB_PATH = "29-01-2020_15h50m29s_Interpolated_Data.pixtab"
    OBS_TIME = [
                "2019-9-7 08:30:10"
                # "2019-9-10 09:00:02",
                # "2019-9-10 09:15:17",
    ...
    """ cargar pixtab completo """
    # pixtab = PixelTable(PIXTAB_PATH)

    """ ubicar estrellas del catalogo en el pixtab """
    # catalog = Catalog(CATALOG_PATH, Time(OBS_TIME))
    # pixtab = PixelTable(PIXTAB_PATH)
    # catalog_pixtab = catalog.find_on_pixtab(pixtab.df) # no es un pixtab completo (util para actualizarlo)
    # catalog_pixtab.to_csv("updatable_catalog_1.pixtab", sep=' ', index=False)
    # catalog_dat = Catalog.catpixtab2dat(catalog_pixtab) # para obtener un dat mergeable (util para crear un pixtab completo interpolable)
    # catalog_dat.to_csv(file_name, sep=' ', index=False)

    """ para visualizar el catalogo segun el pixtab """
    catalog = Catalog(CATALOG_PATH, Time(OBS_TIME))
    pixtab = PixelTable(PIXTAB_PATH)
    catalog_pixtab = catalog.find_on_pixtab(pixtab.df) # (no es un pixtab completo)
    ...
    # updated_catalog_pixtab = catalog_image.update_catalog_pixtab()
    # catalog_dat = catalog_pixtab.catpixtab2dat(updated_catalog_pixtab)
    # catalog_dat.to_csv(file_name, sep=' ', index=False)
